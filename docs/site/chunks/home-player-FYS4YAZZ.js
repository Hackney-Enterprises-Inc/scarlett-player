import{a as Er}from"./chunk-VC46IEJQ.js";var Pe=null;var $t=new WeakMap;function gt(n,e){let t=$t.get(n);t||(t=new Set,$t.set(n,t)),t.add(e)}var Ne=class{constructor(e){this.subscribers=new Set;this.value=e}get(){if(Pe){let e=Pe;this.subscribers.add(e),gt(e,()=>this.subscribers.delete(e))}return this.value}set(e){Object.is(this.value,e)||(this.value=e,this.notify())}update(e){this.set(e(this.value))}subscribe(e){return this.subscribers.add(e),()=>this.subscribers.delete(e)}notify(){Array.from(this.subscribers).forEach(e=>{try{e()}catch(t){console.error("[Scarlett Player] Error in signal subscriber:",t)}})}destroy(){this.subscribers.clear()}getSubscriberCount(){return this.subscribers.size}};function mt(n){return new Ne(n)}var Me={playbackState:"idle",playing:!1,paused:!0,ended:!1,buffering:!1,waiting:!1,seeking:!1,currentTime:0,duration:NaN,buffered:null,bufferedAmount:0,mediaType:"unknown",source:null,title:"",poster:"",volume:1,muted:!1,playbackRate:1,fullscreen:!1,pip:!1,controlsVisible:!0,qualities:[],currentQuality:null,audioTracks:[],currentAudioTrack:null,textTracks:[],currentTextTrack:null,live:!1,liveEdge:!0,seekableRange:null,liveLatency:0,lowLatencyMode:!1,chapters:[],currentChapter:null,error:null,bandwidth:0,autoplay:!1,loop:!1,airplayAvailable:!1,airplayActive:!1,chromecastAvailable:!1,chromecastActive:!1,thumbnails:null,interacting:!1,hovering:!1,focused:!1},Fe=class{constructor(e){this.signals=new Map;this.changeSubscribers=new Set;this.definedDefaults=new Map;this.lastValues=new Map;this.destroyed=!1;this.initializeSignals(e)}initializeSignals(e){let t={...Me,...e};for(let[r,i]of Object.entries(t))this.createSignal(r,i)}createSignal(e,t){let r=mt(t);this.lastValues.set(e,t),r.subscribe(()=>{this.notifyChangeSubscribers(e)}),this.signals.set(e,r)}define(e,t){if(this.signals.has(e)){let r=this.definedDefaults.has(e),i=e in Me,s=r?this.definedDefaults.get(e):Me[e];(r||i)&&!Object.is(s,t)&&console.warn(`[StateManager] State key "${String(e)}" is already defined with a different default. Keeping:`,s,"ignoring:",t);return}this.definedDefaults.set(e,t),this.createSignal(e,t)}get(e){if(this.destroyed)throw new Error(`[StateManager] Manager is destroyed (reading '${e}')`);let t=this.signals.get(e);if(!t)throw new Error(`[StateManager] Unknown state key: ${e}`);return t}getValue(e){return this.get(e).get()}set(e,t){this.get(e).set(t)}update(e){for(let[t,r]of Object.entries(e)){let i=t;this.signals.has(i)&&this.set(i,r)}}subscribeToKey(e,t){let r=this.get(e);return r.subscribe(()=>{t(r.get())})}subscribe(e){return this.changeSubscribers.add(e),()=>this.changeSubscribers.delete(e)}notifyChangeSubscribers(e){let r=this.get(e).get(),i=this.lastValues.get(e);this.lastValues.set(e,r);let s={key:e,value:r,previousValue:i};Array.from(this.changeSubscribers).forEach(o=>{try{o(s)}catch(u){console.error("[StateManager] Error in change subscriber:",u)}})}reset(){this.update({...Me,...Object.fromEntries(this.definedDefaults)})}resetKey(e){let t=e in Me?Me[e]:this.definedDefaults.get(e);this.set(e,t)}snapshot(){let e={};for(let[t,r]of this.signals)e[t]=r.get();return Object.freeze(e)}getSubscriberCount(e){return this.signals.get(e)?.getSubscriberCount()??0}destroy(){this.signals.forEach(e=>e.destroy()),this.signals.clear(),this.changeSubscribers.clear(),this.lastValues.clear(),this.destroyed=!0}};var kr={maxListeners:100,async:!1,interceptors:!0},Oe=class{constructor(e){this.listeners=new Map;this.onceListeners=new Map;this.interceptors=new Map;this.options={...kr,...e}}on(e,t){return this.listeners.has(e)||this.listeners.set(e,new Set),this.listeners.get(e).add(t),this.checkMaxListeners(e),()=>this.off(e,t)}once(e,t){this.onceListeners.has(e)||this.onceListeners.set(e,new Set);let r=this.onceListeners.get(e);return r.add(t),this.listeners.has(e)||this.listeners.set(e,new Set),()=>{r.delete(t)}}off(e,t){let r=this.listeners.get(e);r&&(r.delete(t),r.size===0&&this.listeners.delete(e));let i=this.onceListeners.get(e);i&&(i.delete(t),i.size===0&&this.onceListeners.delete(e))}emit(e,t){let r=this.runInterceptors(e,t);if(r===null)return;let i=this.listeners.get(e);i&&Array.from(i).forEach(u=>{this.safeCallHandler(u,r)});let s=this.onceListeners.get(e);s&&(Array.from(s).forEach(u=>{s.delete(u),this.safeCallHandler(u,r)}),s.size===0&&this.onceListeners.delete(e))}async emitAsync(e,t){let r=await this.runInterceptorsAsync(e,t);if(r===null)return;let i=this.listeners.get(e);if(i){let o=Array.from(i).map(u=>this.safeCallHandlerAsync(u,r));await Promise.all(o)}let s=this.onceListeners.get(e);if(s){let u=Array.from(s).map(a=>(s.delete(a),this.safeCallHandlerAsync(a,r)));await Promise.all(u),s.size===0&&this.onceListeners.delete(e)}}intercept(e,t){if(!this.options.interceptors)return()=>{};this.interceptors.has(e)||this.interceptors.set(e,new Set);let r=this.interceptors.get(e);return r.add(t),()=>{r.delete(t),r.size===0&&this.interceptors.delete(e)}}removeAllListeners(e){e?(this.listeners.delete(e),this.onceListeners.delete(e)):(this.listeners.clear(),this.onceListeners.clear())}listenerCount(e){let t=this.listeners.get(e)?.size??0,r=this.onceListeners.get(e)?.size??0;return t+r}destroy(){this.listeners.clear(),this.onceListeners.clear(),this.interceptors.clear()}runInterceptors(e,t){if(!this.options.interceptors)return t;let r=this.interceptors.get(e);if(!r||r.size===0)return t;let i=t;for(let s of r)try{if(i=s(i),i===null)return null}catch(o){console.error("[EventBus] Error in interceptor:",o)}return i}async runInterceptorsAsync(e,t){if(!this.options.interceptors)return t;let r=this.interceptors.get(e);if(!r||r.size===0)return t;let i=t;for(let s of r)try{let o=s(i);if(i=o instanceof Promise?await o:o,i===null)return null}catch(o){console.error("[EventBus] Error in interceptor:",o)}return i}safeCallHandler(e,t){try{e(t)}catch(r){console.error("[EventBus] Error in event handler:",r)}}async safeCallHandlerAsync(e,t){try{let r=e(t);r instanceof Promise&&await r}catch(r){console.error("[EventBus] Error in event handler:",r)}}checkMaxListeners(e){let t=this.listenerCount(e);t>this.options.maxListeners&&console.warn(`[EventBus] Max listeners (${this.options.maxListeners}) exceeded for event: ${e}. Current count: ${t}. This may indicate a memory leak.`)}};var Wt=["debug","info","warn","error"],Pr=n=>{let t=`${n.scope?`[${n.scope}]`:"[ScarlettPlayer]"} ${n.message}`,r=n.metadata??"";switch(n.level){case"debug":console.debug(t,r);break;case"info":console.info(t,r);break;case"warn":console.warn(t,r);break;case"error":console.error(t,r);break}},Be=class n{constructor(e){this.level=e?.level??"warn",this.scope=e?.scope,this.enabled=e?.enabled??!0,this.handlers=e?.handlers??[Pr]}child(e){return new n({level:this.level,scope:this.scope?`${this.scope}:${e}`:e,enabled:this.enabled,handlers:this.handlers})}debug(e,t){this.log("debug",e,t)}info(e,t){this.log("info",e,t)}warn(e,t){this.log("warn",e,t)}error(e,t){this.log("error",e,t)}setLevel(e){this.level=e}setEnabled(e){this.enabled=e}addHandler(e){this.handlers.push(e)}removeHandler(e){let t=this.handlers.indexOf(e);t!==-1&&this.handlers.splice(t,1)}log(e,t,r){if(!this.enabled||!this.shouldLog(e))return;let i={level:e,message:t,timestamp:Date.now(),scope:this.scope,metadata:r};for(let s of this.handlers)try{s(i)}catch(o){console.error("[Logger] Handler error:",o)}}shouldLog(e){return Wt.indexOf(e)>=Wt.indexOf(this.level)}};var Ve=class{constructor(e,t,r){this.errors=[];this.eventBus=e,this.logger=t,this.maxHistory=r?.maxHistory??10}handle(e,t){let r=this.normalizeError(e,t);return this.addToHistory(r),this.logError(r),this.eventBus.emit("error",r),r}record(e,t){let r=this.normalizeError(e,t);return this.addToHistory(r),this.logError(r),r}throw(e,t,r){let i={code:e,message:t,fatal:r?.fatal??this.isFatalCode(e),timestamp:Date.now(),context:r?.context,originalError:r?.originalError};return this.handle(i,r?.context)}getHistory(){return[...this.errors]}getLastError(){return this.errors[this.errors.length-1]??null}clearHistory(){this.errors=[]}hasFatalError(){return this.errors.some(e=>e.fatal)}normalizeError(e,t){return this.isPlayerError(e)?{...e,context:{...e.context,...t}}:{code:this.getErrorCode(e),message:e.message,fatal:this.isFatal(e,t),timestamp:Date.now(),context:t,originalError:e}}getErrorCode(e){let t=e.message.toLowerCase();return t.includes("quota")?"MEDIA_BUFFER_FULL":t.includes("append")||t.includes("sourcebuffer")||t.includes("arraybuffer")?"MEDIA_APPEND_ERROR":t.includes("network")?"MEDIA_NETWORK_ERROR":t.includes("decode")?"MEDIA_DECODE_ERROR":t.includes("source")?"SOURCE_LOAD_FAILED":t.includes("plugin")?"PLUGIN_SETUP_FAILED":t.includes("provider")?"PROVIDER_SETUP_FAILED":"UNKNOWN_ERROR"}isFatal(e,t){return t?.operation==="load"?!0:this.isFatalCode(this.getErrorCode(e))}isFatalCode(e){return["SOURCE_NOT_SUPPORTED","PROVIDER_NOT_FOUND","MEDIA_DECODE_ERROR"].includes(e)}isPlayerError(e){return typeof e=="object"&&e!==null&&"code"in e&&"message"in e&&"fatal"in e&&"timestamp"in e}addToHistory(e){this.errors.push(e),this.errors.length>this.maxHistory&&this.errors.shift()}logError(e){let t=`[${e.code}] ${e.message}`;e.fatal?this.logger.error(t,{code:e.code,context:e.context}):this.logger.warn(t,{code:e.code,context:e.context})}};var Ue=class{constructor(e,t){this.cleanupFns=[];this.pluginId=e,this.stateManager=t.stateManager,this.eventBus=t.eventBus,this.container=t.container,this.getPluginFn=t.getPlugin,this.logger={debug:(r,i)=>t.logger.debug(`[${e}] ${r}`,i),info:(r,i)=>t.logger.info(`[${e}] ${r}`,i),warn:(r,i)=>t.logger.warn(`[${e}] ${r}`,i),error:(r,i)=>t.logger.error(`[${e}] ${r}`,i)}}getState(e){return this.stateManager.getValue(e)}setState(e,t){this.stateManager.set(e,t)}defineState(e,t){this.stateManager.define(e,t)}on(e,t){return this.eventBus.on(e,t)}off(e,t){this.eventBus.off(e,t)}emit(e,t){this.eventBus.emit(e,t)}getPlugin(e){return this.getPluginFn(e)}onDestroy(e){this.cleanupFns.push(e)}subscribeToState(e){return this.stateManager.subscribe(e)}runCleanups(){for(let e of this.cleanupFns)try{e()}catch(t){this.logger.error("Cleanup function failed",{error:t})}this.cleanupFns=[]}getCleanupFns(){return this.cleanupFns}};var ze=class{constructor(e,t,r,i){this.plugins=new Map;this.initPromises=new Map;this.initializingStack=new Set;this.eventBus=e,this.stateManager=t,this.logger=r,this.container=i.container}register(e,t){if(this.plugins.has(e.id))throw new Error(`Plugin "${e.id}" is already registered`);this.validatePlugin(e);let r=new Ue(e.id,{stateManager:this.stateManager,eventBus:this.eventBus,logger:this.logger,container:this.container,getPlugin:i=>this.getReadyPlugin(i)});this.plugins.set(e.id,{plugin:e,state:"registered",config:t,cleanupFns:[],api:r}),this.logger.info(`Plugin registered: ${e.id}`),this.eventBus.emit("plugin:registered",{name:e.id,type:e.type})}async unregister(e){let t=this.plugins.get(e);t&&(t.state==="ready"&&await this.destroyPlugin(e),this.plugins.delete(e),this.logger.info(`Plugin unregistered: ${e}`))}async initAll(){let e=this.resolveDependencyOrder();for(let t of e)await this.initPlugin(t)}async initPlugin(e){let t=this.plugins.get(e);if(!t)throw new Error(`Plugin "${e}" not found`);if(t.state==="ready")return;if(this.initializingStack.has(e))throw new Error(`Plugin "${e}" is already initializing (possible circular dependency)`);let r=this.initPromises.get(e);if(r)return r;if(t.state==="initializing"){let s=this.initPromises.get(e);return s||void 0}let i=(async()=>{this.initializingStack.add(e);try{for(let s of t.plugin.dependencies||[]){let o=this.plugins.get(s);if(!o)throw new Error(`Plugin "${e}" depends on missing plugin "${s}"`);o.state!=="ready"&&await this.initPlugin(s)}}finally{this.initializingStack.delete(e)}try{if(t.state="initializing",t.plugin.onStateChange){let s=this.stateManager.subscribe(t.plugin.onStateChange.bind(t.plugin));t.api.onDestroy(s)}if(t.plugin.onError){let s=this.eventBus.on("error",o=>{t.plugin.onError?.(o.originalError||new Error(o.message))});t.api.onDestroy(s)}await t.plugin.init(t.api,t.config),t.state="ready",this.logger.info(`Plugin ready: ${e}`),this.eventBus.emit("plugin:active",{name:e})}catch(s){throw t.state="error",t.error=s,this.logger.error(`Plugin init failed: ${e}`,{error:s}),this.eventBus.emit("plugin:error",{name:e,error:s}),s}finally{this.initPromises.delete(e)}})();return this.initPromises.set(e,i),i}async destroyAll(){let e=this.resolveDependencyOrder().reverse();for(let t of e)await this.destroyPlugin(t)}async destroyPlugin(e){let t=this.plugins.get(e);if(!(!t||t.state!=="ready"))try{await t.plugin.destroy()}catch(r){this.logger.error(`Plugin destroy failed: ${e}`,{error:r})}finally{t.api.runCleanups(),t.state="registered",this.eventBus.emit("plugin:destroyed",{name:e})}}getPlugin(e){let t=this.plugins.get(e);return t?t.plugin:null}getReadyPlugin(e){let t=this.plugins.get(e);return t?.state==="ready"?t.plugin:null}hasPlugin(e){return this.plugins.has(e)}getPluginState(e){return this.plugins.get(e)?.state??null}getPluginIds(){return Array.from(this.plugins.keys())}getReadyPlugins(){return Array.from(this.plugins.values()).filter(e=>e.state==="ready").map(e=>e.plugin)}getPluginsByType(e){return Array.from(this.plugins.values()).filter(t=>t.plugin.type===e).map(t=>t.plugin)}selectProvider(e){let t=this.getPluginsByType("provider");for(let r of t){let i=r.canPlay;if(typeof i=="function"&&i(e))return r}return null}resolveDependencyOrder(){let e=new Set,t=new Set,r=[],i=(s,o=[])=>{if(e.has(s))return;if(t.has(s)){let a=[...o,s].join(" -> ");throw new Error(`Circular dependency detected: ${a}`)}let u=this.plugins.get(s);if(u){t.add(s);for(let a of u.plugin.dependencies||[])this.plugins.has(a)&&i(a,[...o,s]);t.delete(s),e.add(s),r.push(s)}};for(let s of this.plugins.keys())i(s);return r}validatePlugin(e){if(!e.id||typeof e.id!="string")throw new Error("Plugin must have a valid id");if(!e.name||typeof e.name!="string")throw new Error(`Plugin "${e.id}" must have a valid name`);if(!e.version||typeof e.version!="string")throw new Error(`Plugin "${e.id}" must have a valid version`);if(!e.type||typeof e.type!="string")throw new Error(`Plugin "${e.id}" must have a valid type`);if(typeof e.init!="function")throw new Error(`Plugin "${e.id}" must have an init() method`);if(typeof e.destroy!="function")throw new Error(`Plugin "${e.id}" must have a destroy() method`)}};function kt(n){return n.querySelector("video")}function Ee(n){let e=document,t=e.fullscreenElement??e.webkitFullscreenElement??null;return t&&n.contains(t)?!0:!!kt(n)?.webkitDisplayingFullscreen}async function we(n){let e=n;if(e.requestFullscreen){await e.requestFullscreen();return}if(e.webkitRequestFullscreen){await e.webkitRequestFullscreen();return}let t=kt(n);if(t?.webkitEnterFullscreen){t.webkitEnterFullscreen();return}throw new Error("Fullscreen is not supported")}async function Le(n){let e=kt(n);if(e?.webkitDisplayingFullscreen){e.webkitExitFullscreen?.();return}let t=document;if(t.exitFullscreen){await t.exitFullscreen();return}t.webkitExitFullscreen&&await t.webkitExitFullscreen()}function j(n){if(n)try{let e=new URL(n);return e.protocol!=="http:"&&e.protocol!=="https:"?void 0:`${e.origin}${e.pathname}`}catch{return}}var ft=class{constructor(e){this._currentProvider=null;this.destroyed=!1;this.seekingWhilePlaying=!1;this.seekResumeTimeout=null;this.initialSrcLoaded=!1;this.loadGeneration=0;this.listenersWired=!1;this.readyEmitted=!1;this.fullscreenAnnounced=!1;this.unwireFullscreen=null;this.destroyPromise=null;this.initializing=null;if(typeof e.container=="string"){let t=document.querySelector(e.container);if(!t||!(t instanceof HTMLElement))throw new Error(`ScarlettPlayer: container not found: ${e.container}`);this.container=t}else if(e.container instanceof HTMLElement)this.container=e.container;else throw new Error("ScarlettPlayer requires a valid HTMLElement container or CSS selector");if(this.initialSrc=e.src,this.eventBus=new Oe,this.stateManager=new Fe({autoplay:e.autoplay??!1,loop:e.loop??!1,volume:e.volume??1,muted:e.muted??!1,poster:e.poster??""}),this.logger=new Be({level:e.logLevel??"warn",scope:"ScarlettPlayer"}),this.errorHandler=new Ve(this.eventBus,this.logger),this.pluginManager=new ze(this.eventBus,this.stateManager,this.logger,{container:this.container}),this.eventBus.on("error",t=>{this.stateManager.set("error",t)}),this.eventBus.on("media:loaded",()=>{this.stateManager.set("error",null)}),this.eventBus.on("media:error",({error:t})=>{this.errorHandler.record(t,{channel:"media:error"})}),this.wireFullscreenListeners(),e.plugins)for(let t of e.plugins)this.pluginManager.register(t);this.logger.info("ScarlettPlayer constructed",{autoplay:e.autoplay,plugins:e.plugins?.length??0})}ensureInitialized(){return this.initializing?this.initializing:(this.initializing=this.runInitialization().finally(()=>{this.initializing=null}),this.initializing)}async runInitialization(){for(let e of this.pluginManager.getPluginIds()){if(this.destroyed)return;let t=this.pluginManager.getPlugin(e);!t||t.type==="provider"||this.pluginManager.getPluginState(e)==="registered"&&await this.pluginManager.initPlugin(e)}this.destroyed||(this.wireLifecycleListeners(),this.readyEmitted||(this.readyEmitted=!0,this.eventBus.emit("player:ready",void 0)))}wireLifecycleListeners(){this.listenersWired||(this.listenersWired=!0,this.eventBus.on("live:seektolive",()=>{this.destroyed||this.seekToLive()}),this.eventBus.on("media:load-request",async({src:e,autoplay:t})=>{this.stateManager.getValue("chromecastActive")||(await this.load(e),!this.destroyed&&t!==!1&&await this.play())}),this.eventBus.on("error:retry",async({src:e})=>{let t=this.stateManager.getValue("live"),r=this.stateManager.getValue("currentTime");await this.load(e),!this.destroyed&&(this.stateManager.getValue("error")||(t?this.seekToLive():r>0&&this.seek(r),!this.destroyed&&await this.play()))}))}async init(){this.checkDestroyed(),await this.ensureInitialized(),this.initialSrc&&!this.initialSrcLoaded&&(this.initialSrcLoaded=!0,await this.load(this.initialSrc))}async load(e){this.checkDestroyed(),this.initialSrcLoaded=!0;let t=++this.loadGeneration;try{if(this.logger.info("Loading source",{source:e}),this.stateManager.update({playing:!1,paused:!0,ended:!1,buffering:!0,currentTime:0,duration:0,bufferedAmount:0,playbackState:"loading",error:null,live:!1}),this._currentProvider){let i=this._currentProvider.id;this.logger.info("Destroying previous provider",{provider:i}),await this.pluginManager.destroyPlugin(i),this._currentProvider=null}if(await this.ensureInitialized(),t!==this.loadGeneration){this.logger.info("Load superseded by newer load call",{source:e});return}let r=this.pluginManager.selectProvider(e);if(!r){this.errorHandler.throw("PROVIDER_NOT_FOUND",`No provider found for source: ${e}`,{fatal:!0,context:{source:e}});return}if(this._currentProvider=r,this.logger.info("Provider selected",{provider:r.id}),await this.pluginManager.initPlugin(r.id),t!==this.loadGeneration){this.logger.info("Load superseded by newer load call",{source:e});return}if(this.stateManager.set("source",{src:e,type:this.detectMimeType(e)}),typeof r.loadSource=="function"&&await r.loadSource(e),t!==this.loadGeneration){this.logger.info("Load superseded by newer load call",{source:e});return}this.stateManager.getValue("autoplay")&&await this.play()}catch(r){t===this.loadGeneration&&(this.stateManager.getValue("error")?this.logger.error("Load failed",{source:j(e),error:r.message}):this.errorHandler.handle(r,{operation:"load",source:j(e)}))}}async play(){this.checkDestroyed();try{this.logger.debug("Play requested"),this.eventBus.emit("playback:play",void 0)}catch(e){this.errorHandler.handle(e,{operation:"play"})}}pause(){this.checkDestroyed();try{this.logger.debug("Pause requested"),this.seekingWhilePlaying=!1,this.seekResumeTimeout!==null&&(clearTimeout(this.seekResumeTimeout),this.seekResumeTimeout=null),this.eventBus.emit("playback:pause",void 0)}catch(e){this.errorHandler.handle(e,{operation:"pause"})}}seek(e){if(this.checkDestroyed(),!Number.isFinite(e)){this.logger.warn("Invalid seek time",{time:e});return}try{this.logger.debug("Seek requested",{time:e}),this.stateManager.getValue("playing")&&(this.seekingWhilePlaying=!0),this.seekResumeTimeout!==null&&(clearTimeout(this.seekResumeTimeout),this.seekResumeTimeout=null),this.eventBus.emit("playback:seeking",{time:e}),this.stateManager.set("currentTime",e),this.seekingWhilePlaying&&(this.seekResumeTimeout=setTimeout(()=>{this.seekingWhilePlaying&&this.stateManager.getValue("playing")&&(this.logger.debug("Resuming playback after seek"),this.seekingWhilePlaying=!1,this.eventBus.emit("playback:play",void 0)),this.seekResumeTimeout=null},300))}catch(t){this.errorHandler.handle(t,{operation:"seek",time:e})}}setVolume(e){this.checkDestroyed();let t=Math.max(0,Math.min(1,e));this.stateManager.set("volume",t),this.eventBus.emit("volume:change",{volume:t,muted:this.stateManager.getValue("muted")})}setMuted(e){this.checkDestroyed(),this.stateManager.set("muted",e),this.eventBus.emit("volume:mute",{muted:e})}setPlaybackRate(e){this.checkDestroyed();let t=Math.max(.0625,Math.min(16,e));this.stateManager.set("playbackRate",t),this.eventBus.emit("playback:ratechange",{rate:t})}setAutoplay(e){this.checkDestroyed(),this.stateManager.set("autoplay",e),this.logger.debug("Autoplay set",{autoplay:e})}setPoster(e){this.checkDestroyed(),this.stateManager.set("poster",e),this.logger.debug("Poster set",{poster:e})}on(e,t){return this.checkDestroyed(),this.eventBus.on(e,t)}once(e,t){return this.checkDestroyed(),this.eventBus.once(e,t)}getPlugin(e){return this.checkDestroyed(),this.pluginManager.getPlugin(e)}registerPlugin(e){this.checkDestroyed(),this.pluginManager.register(e)}getState(){return this.checkDestroyed(),this.stateManager.snapshot()}subscribeToState(e){return this.checkDestroyed(),this.stateManager.subscribe(e)}getQualities(){if(this.checkDestroyed(),!this._currentProvider)return[];let e=this._currentProvider;return typeof e.getLevels=="function"?e.getLevels():[]}setQuality(e){if(this.checkDestroyed(),!this._currentProvider){this.logger.warn("No provider available for quality change");return}let t=this._currentProvider;if(typeof t.setLevel=="function"){if(e!==-1){let r=this.getQualities();if(r.length>0&&(e<0||e>=r.length)){this.logger.warn(`Invalid quality index: ${e} (available: ${r.length})`);return}}t.setLevel(e),this.eventBus.emit("quality:change",{quality:e===-1?"auto":`level-${e}`,auto:e===-1})}}getCurrentQuality(){if(this.checkDestroyed(),!this._currentProvider)return-1;let e=this._currentProvider;return typeof e.getCurrentLevel=="function"?e.getCurrentLevel():-1}wireFullscreenListeners(){let e=()=>{this.fullscreenAnnounced=!0,this.setFullscreenState(Ee(this.container))};document.addEventListener("fullscreenchange",e),document.addEventListener("webkitfullscreenchange",e),this.container.addEventListener("webkitbeginfullscreen",e,!0),this.container.addEventListener("webkitendfullscreen",e,!0),this.unwireFullscreen=()=>{document.removeEventListener("fullscreenchange",e),document.removeEventListener("webkitfullscreenchange",e),this.container.removeEventListener("webkitbeginfullscreen",e,!0),this.container.removeEventListener("webkitendfullscreen",e,!0)}}setFullscreenState(e){this.stateManager.getValue("fullscreen")!==e&&(this.stateManager.set("fullscreen",e),this.eventBus.emit("fullscreen:change",{fullscreen:e}))}async requestFullscreen(){this.checkDestroyed(),this.fullscreenAnnounced=!1;try{await we(this.container),this.fullscreenAnnounced||(this.stateManager.set("fullscreen",!0),this.eventBus.emit("fullscreen:change",{fullscreen:!0}))}catch(e){this.logger.error("Fullscreen request failed",{error:e})}}async exitFullscreen(){this.checkDestroyed(),this.fullscreenAnnounced=!1;try{await Le(this.container),this.fullscreenAnnounced||(this.stateManager.set("fullscreen",!1),this.eventBus.emit("fullscreen:change",{fullscreen:!1}))}catch(e){this.logger.error("Exit fullscreen failed",{error:e})}}async toggleFullscreen(){this.fullscreen?await this.exitFullscreen():await this.requestFullscreen()}requestAirPlay(){this.checkDestroyed();let e=this.pluginManager.getPlugin("airplay");e&&typeof e.showPicker=="function"?e.showPicker():this.logger.warn("AirPlay plugin not available")}async requestChromecast(){this.checkDestroyed();let e=this.pluginManager.getPlugin("chromecast");e&&typeof e.requestSession=="function"?await e.requestSession():this.logger.warn("Chromecast plugin not available")}stopCasting(){this.checkDestroyed();let e=this.pluginManager.getPlugin("airplay");e&&typeof e.stop=="function"&&e.stop();let t=this.pluginManager.getPlugin("chromecast");t&&typeof t.stopSession=="function"&&t.stopSession()}seekToLive(){if(this.checkDestroyed(),!this.stateManager.getValue("live")){this.logger.warn("Not a live stream");return}if(this._currentProvider){let i=this._currentProvider;if(typeof i.getLiveInfo=="function"){let s=i.getLiveInfo();if(s?.liveSyncPosition!==void 0){this.seek(s.liveSyncPosition);return}}}let t=this.stateManager.getValue("seekableRange");if(t?.end!==void 0){this.seek(t.end);return}let r=this.stateManager.getValue("duration");Number.isFinite(r)&&r>0&&this.seek(r)}destroy(){return this.destroyPromise?this.destroyPromise:this.destroyed?Promise.resolve():(this.destroyPromise=(async()=>{this.logger.info("Destroying player"),this.destroyed=!0,this.loadGeneration++,this.seekResumeTimeout!==null&&(clearTimeout(this.seekResumeTimeout),this.seekResumeTimeout=null),this.unwireFullscreen?.(),this.unwireFullscreen=null,this.eventBus.emit("player:destroy",void 0);try{await this.pluginManager.destroyAll()}catch(e){this.logger.error("Error during plugin destruction",e)}this.eventBus.destroy(),this.stateManager.destroy(),this.logger.info("Player destroyed")})(),this.destroyPromise)}get playing(){return this.stateManager.getValue("playing")}get paused(){return this.stateManager.getValue("paused")}get currentTime(){return this.stateManager.getValue("currentTime")}get duration(){return this.stateManager.getValue("duration")}get volume(){return this.stateManager.getValue("volume")}get muted(){return this.stateManager.getValue("muted")}get playbackRate(){return this.stateManager.getValue("playbackRate")}get bufferedAmount(){return this.stateManager.getValue("bufferedAmount")}get currentProvider(){return this._currentProvider}get fullscreen(){return this.stateManager.getValue("fullscreen")}get live(){return this.stateManager.getValue("live")}get autoplay(){return this.stateManager.getValue("autoplay")}get poster(){return this.stateManager.getValue("poster")}checkDestroyed(){if(this.destroyed)throw new Error("Cannot call methods on destroyed player")}detectMimeType(e){let t=e;try{t=new URL(e).pathname}catch{let i=e.split("?")[0]??e;t=i.split("#")[0]??i}switch(t.split(".").pop()?.toLowerCase()??""){case"m3u8":return"application/x-mpegURL";case"mpd":return"application/dash+xml";case"mp4":case"m4v":return"video/mp4";case"webm":return"video/webm";case"ogg":case"ogv":return"video/ogg";case"mov":return"video/quicktime";case"mkv":return"video/x-matroska";case"mp3":return"audio/mpeg";case"wav":return"audio/wav";case"flac":return"audio/flac";case"aac":case"m4a":return"audio/mp4";default:return"video/mp4"}}};async function Mt(n){let e=new ft(n);return await e.init(),e}function _t(n){return n<10?`0${n}`:`${n}`}function le(n){if(!isFinite(n)||isNaN(n))return"0:00";let e=Math.abs(n),t=Math.floor(e/3600),r=Math.floor(e%3600/60),i=Math.floor(e%60),s=n<0?"-":"";return t>0?`${s}${t}:${_t(r)}:${_t(i)}`:`${s}${r}:${_t(i)}`}function pe(n){return!Number.isFinite(n)||n<=0?"LIVE":`-${le(n)}`}var $e={play:"M8 5v14l11-7z",volumeHigh:"M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z",volumeMuted:"M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"};var vt=new Map;function At(n,e){if(typeof document>"u")return()=>{};let t=document.getElementById(n),r=vt.get(n);(!r||!t)&&(r={generation:(r?.generation??0)+1,holders:0,element:t},vt.set(n,r)),r.holders+=1;let{generation:i}=r;if(!t){let o=document.createElement("style");o.id=n,o.textContent=e,document.head.appendChild(o),r.element=o}let s=!1;return()=>{if(s)return;s=!0;let o=vt.get(n);!o||o.generation!==i||(o.holders-=1,!(o.holders>0)&&(vt.delete(n),o.element?.remove()))}}var Ct={};Er(Ct,{createHlsInstance:()=>Ar,getHlsConstructor:()=>Cr,isHLSSupported:()=>Mr,isHlsJsSupported:()=>Qt,loadHlsJs:()=>_r,resetLoader:()=>Hr,supportsNativeHLS:()=>Gt});var ce=null,_e=null;function Gt(){return typeof document>"u"?!1:document.createElement("video").canPlayType("application/vnd.apple.mpegurl")!==""}function Qt(){return ce?ce.isSupported():typeof window>"u"?!1:!!(window.MediaSource||window.WebKitMediaSource)}function Mr(){return Gt()||Qt()}async function _r(){return ce||_e||(_e=(async()=>{try{if(ce=(await import("./hls-SXYGB37P.js")).default,!ce.isSupported())throw new Error("hls.js is not supported in this browser");return ce}catch(n){throw _e=null,new Error(`Failed to load hls.js: ${n instanceof Error?n.message:"Unknown error"}`)}})(),_e)}function Ar(n){if(!ce)throw new Error("hls.js is not loaded. Call loadHlsJs() first.");return new ce(n)}function Cr(){return ce}function Hr(){ce=null,_e=null}function fe(n){if(n.name)return n.name;if(n.height){let e={2160:"4K",1440:"1440p",1080:"1080p",720:"720p",480:"480p",360:"360p",240:"240p",144:"144p"},t=Object.keys(e).map(Number).sort((r,i)=>Math.abs(r-n.height)-Math.abs(i-n.height))[0];return Math.abs(t-n.height)<=20?e[t]:`${n.height}p`}return n.bitrate?Rr(n.bitrate):"Unknown"}function Rr(n){return n>=1e6?`${(n/1e6).toFixed(1)} Mbps`:n>=1e3?`${Math.round(n/1e3)} Kbps`:`${n} bps`}function Yt(n,e){return n.map((t,r)=>({index:r,width:t.width||0,height:t.height||0,bitrate:t.bitrate||0,label:fe(t),codec:t.codecSet}))}function jt(n){if(n!==void 0&&n>0)return n;let t=navigator.connection;if(t?.downlink&&t.downlink>0){let r=t.downlink*1e6;return Math.round(r*.85)}return 5e5}function ue(n){return typeof n=="number"&&Number.isFinite(n)?n:null}function Dr(n){if(!n)return null;let e=ue(n.fragmentStart)??ue(n.fragments?.[0]?.start)??0,t=ue(n.totalduration),r=ue(n.edge)??(t===null?null:e+t);return r===null?null:{start:e,end:r}}function Xt(n){let e=n?.seekable;if(!e||e.length===0)return null;let t=e.start(0),r=e.end(e.length-1);return!Number.isFinite(t)||!Number.isFinite(r)?null:{start:t,end:r}}function Ir(n){if(!n)return null;let e=ue(n.targetduration);return ue(n.partHoldBack)??ue(n.holdBack)??(e!==null?e*3:null)}function Nr(n,e){let t=ue(n?.targetduration),r=t!==null?t/2:e/2;return Math.max(1.5,ue(n?.partTarget)??r)}function xe(n){if(n.kind==="hls"){let{hls:o,details:u}=n,a=o.media,d=Dr(u)??Xt(a),y=ue(o.targetLatency)??Ir(u)??3,P=ue(o.latency)??(d&&a?Math.max(0,d.end-a.currentTime):null);if(P===null&&d===null)return null;let A=P??0;return{latency:A,targetLatency:y,atEdge:A<=y+Nr(u,y),seekableRange:d,lowLatency:n.lowLatencyRequested!==!1&&(!!u?.partList?.length||u?.canBlockReload===!0)}}let{media:e}=n,t=Xt(e);if(!t)return null;let r=n.targetLatency??3,i=n.targetLatency===void 0?7:Math.max(1.5,n.targetLatency/2),s=Math.max(0,t.end-e.currentTime);return{latency:s,targetLatency:r,atEdge:s<=r+i,seekableRange:t,lowLatency:n.lowLatency===!0}}function Ht(n,e){if(!e)return;let t=n.getState("liveLatency");Math.abs(t-e.latency)>.05&&(n.setState("liveLatency",e.latency),n.emit("live:latency",{latency:e.latency})),n.getState("liveEdge")!==e.atEdge&&(n.setState("liveEdge",e.atEdge),n.emit("live:edgechange",{atEdge:e.atEdge}));let r=e.seekableRange;if(r){let i=n.getState("seekableRange");(!i||Math.abs(i.start-r.start)>.05||Math.abs(i.end-r.end)>.05)&&(n.setState("seekableRange",{start:r.start,end:r.end}),n.emit("live:seekablerange",{start:r.start,end:r.end}))}n.getState("lowLatencyMode")!==e.lowLatency&&(n.setState("lowLatencyMode",e.lowLatency),n.emit("live:lowlatency",{enabled:e.lowLatency}))}function yt(n){n.getState("lowLatencyMode")&&(n.setState("lowLatencyMode",!1),n.emit("live:lowlatency",{enabled:!1})),n.setState("liveLatency",0),n.setState("liveEdge",!1),n.setState("seekableRange",null)}var Rt={NETWORK_ERROR:"networkError",MEDIA_ERROR:"mediaError",KEY_SYSTEM_ERROR:"keySystemError",MUX_ERROR:"muxError",OTHER_ERROR:"otherError"};function Fr(n){switch(n){case Rt.NETWORK_ERROR:return"network";case Rt.MEDIA_ERROR:return"media";case Rt.MUX_ERROR:return"mux";default:return"other"}}function Or(n){let e=n.frag,t=n.response,r=n.context;return{type:Fr(n.type),details:n.details||"Unknown error",fatal:n.fatal||!1,url:n.url??e?.url??t?.url??r?.url,reason:n.reason,response:n.response}}function Br(n){return`audio-${n}`}function Jt(n){if(!n)return-1;let e=/^audio-(0|[1-9]\d*)$/.exec(n);return e?Number.parseInt(e[1],10):-1}function Vr(n,e,t){return{id:Br(e),label:n.name||n.lang||`Audio ${e+1}`,language:n.lang,active:t}}function Zt(n,e,t){let r=[],i=(u,a)=>{n.on(u,a),r.push({event:u,handler:a})};i("hlsManifestParsed",(u,a)=>{e.logger.debug("HLS manifest parsed",{levels:a.levels.length});let d=a.levels.map((y,P)=>({id:`level-${P}`,label:fe(y),width:y.width,height:y.height,bitrate:y.bitrate,active:P===n.currentLevel}));e.setState("qualities",d),e.emit("quality:levels",{levels:d.map(y=>({id:y.id,label:y.label}))}),t.onManifestParsed?.(a.levels,a)}),i("hlsLevelSwitched",(u,a)=>{let d=n.levels[a.level],y=t.getIsAutoQuality?.()??n.autoLevelEnabled;if(e.logger.debug("HLS level switched",{level:a.level,height:d?.height,auto:y}),d){let P=y?`Auto (${fe(d)})`:fe(d);e.setState("currentQuality",{id:y?"auto":`level-${a.level}`,label:P,width:d.width,height:d.height,bitrate:d.bitrate,active:!0})}e.emit("quality:change",{quality:d?fe(d):"auto",auto:y}),t.onLevelSwitched?.(a.level)});let s=(u,a)=>{let d=u.map((y,P)=>Vr(y,P,P===a));e.setState("audioTracks",d),e.setState("currentAudioTrack",d[a]??null)};i("hlsAudioTracksUpdated",(u,a)=>{let d=a.audioTracks??[];e.logger.debug("HLS audio tracks updated",{tracks:d.length}),s(d,n.audioTrack),t.onAudioTracksUpdated?.(d)}),i("hlsAudioTrackSwitched",(u,a)=>{e.logger.debug("HLS audio track switched",{id:a.id}),s(n.audioTracks??[],a.id),t.onAudioTrackSwitched?.(a.id)});let o=0;return i("hlsFragLoaded",()=>{let u=Date.now();u-o>=2e3&&n.bandwidthEstimate&&(o=u,e.setState("bandwidth",Math.round(n.bandwidthEstimate))),t.onFragLoaded?.()}),i("hlsFragBuffered",()=>{e.setState("buffering",!1),t.onBufferUpdate?.()}),i("hlsFragLoading",()=>{e.setState("buffering",!0)}),i("hlsLevelLoaded",(u,a)=>{a.details?.live!==void 0&&(e.setState("live",a.details.live),a.details.live?(t.onLevelDetails?.(a.details),Ht(e,xe({kind:"hls",hls:n,details:a.details,lowLatencyRequested:t.isLowLatencyRequested?.()??!0}))):yt(e),t.onLiveUpdate?.())}),i("hlsError",(u,a)=>{let d=Or(a);!d.fatal&&(d.details?.includes("bufferStalledError")||a.reason?.includes("buffer holes"))?e.logger.debug(`HLS buffer recovery: ${d.reason||d.details}`,{details:d.details,reason:d.reason}):d.fatal?e.logger.error(`HLS fatal error: ${d.details} (type=${d.type})`,{type:d.type,details:d.details,status:d.response?.code,url:j(d.url)}):e.logger.warn(`HLS error: ${d.details} (type=${d.type}, fatal=${d.fatal})`,{type:d.type,details:d.details,fatal:d.fatal,status:d.response?.code,url:j(d.url)}),t.onError?.(d)}),()=>{for(let{event:u,handler:a}of r)n.off(u,a);r.length=0,e.setState("audioTracks",[]),e.setState("currentAudioTrack",null)}}function Dt(n,e,t,r){let i=[],s=(a,d)=>{n.addEventListener(a,d),i.push({event:a,handler:d})},o=()=>{n.ended||!e.getState("ended")||(e.setState("ended",!1),e.setState("playbackState",n.paused?"paused":"playing"))};s("play",()=>{e.setState("paused",!1),o()}),s("playing",()=>{e.setState("playing",!0),e.setState("paused",!1),e.setState("waiting",!1),e.setState("buffering",!1),e.setState("playbackState","playing"),o(),r&&(r.corePauseRequested=!1),r?.corePlayRequested?r.corePlayRequested=!1:e.emit("playback:play",void 0)}),s("pause",()=>{e.setState("playing",!1),e.setState("paused",!0),e.setState("playbackState","paused"),r&&(r.corePlayRequested=!1),r?.corePauseRequested?r.corePauseRequested=!1:e.emit("playback:pause",void 0)}),s("ended",()=>{e.setState("playing",!1),e.setState("ended",!0),e.setState("playbackState","ended"),e.emit("playback:ended",void 0)}),s("timeupdate",()=>{e.setState("currentTime",n.currentTime),e.emit("playback:timeupdate",{currentTime:n.currentTime}),(e.getState("live")||!Number.isFinite(n.duration))&&Ht(e,t?t():xe({kind:"media",media:n}))}),s("durationchange",()=>{let a=n.duration;!Number.isFinite(a)||a===1/0?(e.setState("live",!0),e.setState("duration",0)):e.setState("duration",a||0),e.emit("media:loadedmetadata",{duration:e.getState("duration")})}),s("waiting",()=>{e.setState("waiting",!0),e.setState("buffering",!0),e.emit("media:waiting",void 0)}),s("canplay",()=>{e.setState("waiting",!1),e.setState("playbackState","ready"),e.emit("media:canplay",void 0)}),s("canplaythrough",()=>{e.setState("buffering",!1),e.emit("media:canplaythrough",void 0)}),s("progress",()=>{if(n.buffered.length>0){let a=n.buffered.end(n.buffered.length-1),d=n.duration>0?a/n.duration:0;e.setState("bufferedAmount",d),e.setState("buffered",n.buffered),e.emit("media:progress",{buffered:d})}}),s("seeking",()=>{e.setState("seeking",!0),o()}),s("seeked",()=>{e.setState("seeking",!1),e.emit("playback:seeked",{time:n.currentTime})}),s("volumechange",()=>{e.setState("volume",n.volume),e.setState("muted",n.muted),e.emit("volume:change",{volume:n.volume,muted:n.muted})}),s("ratechange",()=>{e.setState("playbackRate",n.playbackRate),e.emit("playback:ratechange",{rate:n.playbackRate})}),s("loadedmetadata",()=>{e.setState("duration",n.duration)}),s("error",()=>{let a=n.error;a&&(e.logger.error("Video element error",{code:a.code,message:a.message}),e.emit("media:error",{error:new Error(a.message||"Video playback error")}))}),s("enterpictureinpicture",()=>{e.setState("pip",!0),e.logger.debug("PiP: entered (standard)")}),s("leavepictureinpicture",()=>{e.setState("pip",!1),e.logger.debug("PiP: exited (standard)"),(!n.paused||e.getState("playing"))&&n.play().catch(()=>{})});let u=n;return"webkitPresentationMode"in n&&s("webkitpresentationmodechanged",()=>{let a=u.webkitPresentationMode,d=a==="picture-in-picture";e.setState("pip",d),e.logger.debug(`PiP: mode changed to ${a} (webkit)`),a==="inline"&&n.paused&&n.play().catch(()=>{})}),()=>{for(let{event:a,handler:d}of i)n.removeEventListener(a,d);i.length=0}}var Ur=["loadedmetadata","loadeddata","resize","playing"],zr=["addtrack","removetrack","change"];function It(n){return typeof n=="object"&&n!==null&&typeof n.length=="number"}function er(n){let e=null,t=!1,r=!1,i=!1,s=null,o=[],u=null,a=!1,d=()=>t?"video":r?"audio":"unknown",y=()=>{let T=d();T!==u&&(u=T,n.setState("mediaType",T))},P=()=>{let T=s;if(!T)return;if(i&&T.videoWidth>0){t=!0;return}if(T.readyState<1)return;let I=T.videoTracks,H=T.audioTracks;if(It(I)){if(I.length>0){t=!0;return}It(H)&&H.length>0&&(r=!0)}},A=()=>{a||(P(),y())},q=()=>{for(let T of o)T();o=[],s=null};return{beginSource(T){a||e!==T&&(e=T,t=!1,r=!1,i=!1,u=null,y())},attach(T){if(!a){q(),s=T,i=!1;for(let I of Ur){let H=()=>{i=!0,A()};T.addEventListener(I,H),o.push(()=>T.removeEventListener(I,H))}for(let I of[s.videoTracks,s.audioTracks])if(!(!It(I)||typeof I.addEventListener!="function"))for(let H of zr){let w=()=>A();I.addEventListener(H,w),o.push(()=>I.removeEventListener?.(H,w))}A()}},noteManifestParsed(T){if(a||!T)return;let I=typeof T.video=="boolean"?T.video:null,H=typeof T.audio=="boolean"?T.audio:null;I===!0?t=!0:I===!1&&H===!0&&(r=!0),y()},evaluate:A,current(){return d()},destroy(){a||(a=!0,q())}}}var Nt="Invalid playlist document",$r=["level","audioTrack","subtitleTrack"];function Kr(n,e){if(typeof n!="string"||n.length===0)return!1;let t=n.trimStart();return t.startsWith("#EXTM3U")?e&&$r.includes(e)?/^#EXT(?:INF|-X-TARGETDURATION):/m.test(t):!0:!1}function tr(n){let e=n.DefaultConfig.loader;return class extends e{load(r,i,s){let o={...s,onSuccess:(u,a,d,y)=>{if(!Kr(u?.data,d?.type)){s.onError({code:0,text:Nt},d,y,a);return}s.onSuccess(u,a,d,y)}};super.load(r,i,o)}}}var rr=typeof __PKG_VERSION__<"u"?__PKG_VERSION__:"0.0.0-dev";var qr={debug:!1,autoStartLoad:!0,startPosition:-1,lowLatencyMode:!1,maxBufferLength:30,maxMaxBufferLength:600,backBufferLength:30,enableWorker:!0,capLevelToPlayerSize:!0,maxNetworkRetries:3,maxMediaRetries:2,retryDelayMs:1e3,retryBackoffFactor:2,loadTimeoutMs:3e4,autoReconnect:!0,reconnectBaseDelayMs:2e3,reconnectMaxDelayMs:3e4,reconnectWindowMs:3e5,validatePlaylists:!0},Wr=1.1,Gr=["manifestLoadError","manifestLoadTimeOut","manifestParsingError"];function ir(n,e,t){let r={...qr,...t},i=null,s=null,o=null,u=!1,a=null,d=null,y=null,P=!0,A={corePlayRequested:!1,corePauseRequested:!1},q=null,T=null,I=null,H=null,w=0,W=null,k=0,C=0,U=null,re=0,f=0,V=10,G=5e3,L=!1,N=null,O=0,x=0,R=0,S=null,F=null,$=!1,ie=!1,de=!1,X=null,Q=0,he=0,Se=()=>{let p=s&&!u?xe({kind:"hls",hls:s,details:T,lowLatencyRequested:r.lowLatencyMode===!0}):o?xe({kind:"media",media:o,targetLatency:H??void 0,lowLatency:I?.lowLatency}):null;return p&&(I=p,s&&!u&&(H=p.targetLatency)),p},Te=()=>{o&&(o.poster=i?.getState("poster")||"")},He=()=>{if(o)return o;let p=i?.container.querySelector("video");return p?(o=p,o):(o=document.createElement("video"),o.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;display:block;object-fit:contain;background:#000",o.preload="metadata",o.controls=!1,o.playsInline=!0,Te(),i?.container.appendChild(o),o)},ge=p=>{W?.(p??new Error("HLS load cancelled")),W=null,d?.(),d=null,y?.(),y=null,U&&(clearTimeout(U),U=null),X&&(clearTimeout(X),X=null),s&&(s.destroy(),s=null)},ke=p=>{ge(p),a=null,u=!1,P=!0,k=0,C=0,re=0,f=0,T=null,I=null,H=null,i&&yt(i)},ot=()=>{let p=Lt();if(r.validatePlaylists!==!1){let c=n.getHlsConstructor();c&&c.DefaultConfig?.loader&&(p.pLoader=tr(c))}return p},wt=()=>{let p={},c=(Z,se)=>{se!==void 0&&(p[Z]=se)},l=r.liveSyncDuration,h=r.liveSyncDurationCount,g=r.liveMaxLatencyDuration,m=r.liveMaxLatencyDurationCount,v=(l!==void 0||g!==void 0)&&(h!==void 0||m!==void 0),_=v&&l!==void 0,Y=v&&!_;return v&&i?.logger.warn(`Ignoring ${_?"liveSyncDurationCount/liveMaxLatencyDurationCount":"liveSyncDuration/liveMaxLatencyDuration"}: hls.js rejects a config mixing seconds-based and count-based live latency options`),c("liveSyncDuration",Y?void 0:l),c("liveSyncDurationCount",_?void 0:h),c("liveMaxLatencyDuration",Y?void 0:g),c("liveMaxLatencyDurationCount",_?void 0:m),c("liveDurationInfinity",r.liveDurationInfinity),c("maxLiveSyncPlaybackRate",r.maxLiveSyncPlaybackRate??(r.lowLatencyMode===!0?Wr:void 0)),p},Lt=()=>({debug:r.debug,autoStartLoad:r.autoStartLoad,startPosition:r.startPosition,startLevel:-1,abrEwmaDefaultEstimate:jt(r.initialBandwidthEstimate),lowLatencyMode:r.lowLatencyMode,maxBufferLength:r.maxBufferLength,maxMaxBufferLength:r.maxMaxBufferLength,backBufferLength:r.backBufferLength,enableWorker:r.enableWorker,capLevelToPlayerSize:r.capLevelToPlayerSize,fragLoadingMaxRetry:1,manifestLoadingMaxRetry:1,levelLoadingMaxRetry:1,fragLoadingRetryDelay:500,manifestLoadingRetryDelay:500,levelLoadingRetryDelay:500,...wt()}),Re=p=>{let c=r.retryDelayMs??1e3,l=r.retryBackoffFactor??2;return c*Math.pow(l,p)*(.7+Math.random()*.3)},xt=["bufferAppendError","bufferAppendingError","bufferAddCodecError"],at=p=>{if(p.response?.text===Nt)return"PLAYLIST_INVALID";if(p.details==="bufferFullError")return"MEDIA_BUFFER_FULL";if(xt.includes(p.details))return"MEDIA_APPEND_ERROR";switch(p.type){case"network":return"MEDIA_NETWORK_ERROR";case"media":case"mux":return"MEDIA_DECODE_ERROR";default:return"PLAYBACK_FAILED"}},lt=(p,c)=>{let l=p.type==="network"?k:p.type==="media"?C:0,h={type:p.type,retriesExhausted:c,attempts:l};typeof p.response?.code=="number"&&p.response.code>0&&(h.httpStatus=p.response.code);let g=j(p.url);return g&&(h.url=g),h},me=(p,c)=>{let l=c?`HLS error: ${p.details} (max retries exceeded)`:`HLS error: ${p.details}`;i?.logger.error(l,{type:p.type,details:p.details}),i?.setState("playbackState","error"),i?.setState("buffering",!1),i?.emit("error",{code:at(p),message:l,fatal:!0,timestamp:Date.now(),detail:lt(p,c)}),ne(p)},St=p=>{if(!n.getHlsConstructor()||!s)return!1;if(p.fatal){let l=Date.now();if(l-f>G?(re=1,f=l):re++,re>=V)return i?.logger.error(`Too many fatal errors (${re} in ${G}ms), giving up`),me(p,!0),ge(new Error(p.details)),!0;switch(i?.logger.error("Fatal HLS error",{type:p.type,details:p.details}),p.type){case"network":{let h=r.maxNetworkRetries??3;if(k>=h)return i?.logger.error(`Network error recovery failed after ${k} attempts`),me(p,!0),!0;k++;let g=Re(k-1);i?.logger.info(`Attempting network error recovery (attempt ${k}/${h}) in ${g}ms`),i?.emit("error:network",{error:new Error(p.details)}),U&&clearTimeout(U);let m=Gr.includes(p.details),v=w;U=setTimeout(()=>{v!==w||!s||(m&&a?s.loadSource(a):s.startLoad())},g);break}case"media":{let h=r.maxMediaRetries??2;if(C>=h)return i?.logger.error(`Media error recovery failed after ${C} attempts`),me(p,!0),!0;C++;let g=Re(C-1);i?.logger.info(`Attempting media error recovery (attempt ${C}/${h}) in ${g}ms`),i?.emit("error:media",{error:new Error(p.details)}),U&&clearTimeout(U);let m=w;U=setTimeout(()=>{m!==w||!s||s.recoverMediaError()},g);break}default:return me(p,!1),!0}}return!1},ct=(p,c)=>{let l=p.type==="network",h=l?r.maxNetworkRetries??3:r.maxMediaRetries??2,g=l?k:C;if(!a||g>=h){me(p,g>=h);return}let m=c??o?.currentTime??0;l?k++:C++;let v=g+1,_=Re(v-1);i?.logger.info(`Attempting native ${p.type} error recovery (attempt ${v}/${h}) in ${_}ms`),i?.emit(l?"error:network":"error:media",{error:new Error(p.details)}),U&&clearTimeout(U);let Y=w;U=setTimeout(()=>{Y===w&&De(p,m)},_)},De=async(p,c)=>{if(!a)return;let l=++w,h=a,g=i?.getState("live")??!1;try{if(ge(new Error("HLS load cancelled: native error recovery")),i?.setState("playbackState","loading"),await ee(h),l!==w)return;!g&&o&&c>0&&(o.currentTime=c),i?.setState("playbackState","ready"),i?.setState("buffering",!1);try{await o?.play()}catch{}}catch{if(l!==w)return;i?.logger.warn("Native error recovery attempt failed"),ct(p,c)}},ee=async p=>{let c=w,l=He();return u=!0,i&&(y=Dt(l,i,Se,A)),q?.beginSource(p),q?.attach(l),new Promise((h,g)=>{let m=null,v=!1,_=()=>{v=!0,W===Y&&(W=null),l.removeEventListener("loadedmetadata",Z),l.removeEventListener("error",se),m!==null&&(clearTimeout(m),m=null)},Y=M=>{v||(_(),g(M))};W=Y;let Z=()=>{if(v)return;if(c!==w){_(),g(new Error("HLS load cancelled"));return}_(),L=!0;let M=()=>{let ht=l.error,br={type:ht?.code===MediaError.MEDIA_ERR_NETWORK?"network":"media",details:ht?.message||"Native HLS playback error",fatal:!0};ct(br)};l.addEventListener("error",M);let oe=()=>{(k>0||C>0)&&(i?.logger.debug("Native playback recovered, resetting retry budgets"),k=0,C=0)};l.addEventListener("playing",oe);let ae=()=>{l.removeEventListener("error",M),l.removeEventListener("playing",oe)},be=y;y=()=>{ae(),be?.()},i?.setState("source",{src:p,type:"application/x-mpegURL"}),i?.emit("media:loaded",{src:p,type:"application/x-mpegURL"}),h()},se=()=>{if(v)return;_();let M=l.error;g(new Error(M?.message||"Failed to load HLS source"))},J=r.loadTimeoutMs??3e4;J>0&&(m=setTimeout(()=>{v||c!==w||(_(),g(new Error("Video took too long to load (network timeout)")))},J)),l.addEventListener("loadedmetadata",Z),l.addEventListener("error",se),l.src=p,l.load()})},Ie=async p=>{let c=w;if(await n.loadHlsJs(),c!==w)throw new Error("HLS load cancelled");let l=He();return u=!1,s=n.createHlsInstance(ot()),i&&(y=Dt(l,i,Se,A)),q?.beginSource(p),q?.attach(l),new Promise((h,g)=>{if(!s||!i){g(new Error("HLS not initialized"));return}let m=!1,v=null,_=()=>{v!==null&&(clearTimeout(v),v=null)},Y=J=>{m||(m=!0,_(),g(J))};W=Y;let Z=()=>{W===Y&&(W=null)};d=Zt(s,i,{onManifestParsed:(J,M)=>{c===w&&(q?.noteManifestParsed(M),m||(m=!0,Z(),_(),L=!0,i?.setState("source",{src:p,type:"application/x-mpegURL"}),i?.emit("media:loaded",{src:p,type:"application/x-mpegURL"}),h()))},onLevelSwitched:()=>{},onLevelDetails:J=>{c===w&&(T=J,Se())},isLowLatencyRequested:()=>r.lowLatencyMode===!0,onError:J=>{if(c!==w)return;St(J)&&!m&&(m=!0,Z(),_(),g(new Error(J.details)))},onFragLoaded:()=>{c===w&&(k>0||C>0)&&(i?.logger.debug("Playback recovered, resetting retry budgets"),k=0,C=0)},getIsAutoQuality:()=>P});let se=r.loadTimeoutMs??3e4;se>0&&(v=setTimeout(()=>{if(m||c!==w)return;m=!0,Z(),i?.logger.error(`HLS load timed out after ${se}ms`,{src:j(p)});let J={type:"network",details:"Video took too long to load (network timeout)",fatal:!0};me(J,!0),ge(),g(new Error(J.details))},se)),s.attachMedia(l),s.loadSource(p)})},te=()=>{N&&(clearTimeout(N),N=null),O=0,x=0,R=0,F=null,$=!1,ie=!1},ut=()=>{if(!o||X)return;Q=Date.now(),he=o.currentTime;let p=s?.targetDuration??s?.levels?.[s?.currentLevel]?.details?.targetduration??0,c=Math.max(15,4*(p||0)||15),l=Math.min(c,3e4),h=()=>{if(!o||!i)return;let g=i.getState("playing"),m=i.getState("seeking");if(!g||m){X=setTimeout(h,l);return}let v=Date.now()-Q;if(o.currentTime-he>.5)Q=Date.now(),he=o.currentTime;else if(v>c*1e3){i.logger.warn("Playback stall detected \u2014 no timeupdate progress",{elapsed:Math.round(v),currentTime:o.currentTime,targetDuration:Math.round(c)}),ne({type:"network",details:"Playback stalled \u2014 no data received",fatal:!0});return}X=setTimeout(h,l)};X=setTimeout(h,l)},ve=()=>{X&&(clearTimeout(X),X=null)},dt=(p,c)=>{if($)return;$=!0;let l=O,h=F;i?.emit("error:reconnect-exhausted",{attempts:l,elapsedMs:p,windowMs:c}),i?.setState("playbackState","error"),i?.setState("buffering",!1),i?.emit("error",{code:h?at(h):"PLAYBACK_FAILED",message:`HLS auto-reconnect gave up after ${l} attempts over ${Math.round(p/1e3)}s`,fatal:!0,timestamp:Date.now(),detail:{type:h?.type??"other",retriesExhausted:!0,attempts:l,reconnectExhausted:!0}})},ye=()=>{if($||N)return;let p=(i?.getState("live")??!1)&&de,c=r.reconnectWindowMs??3e5,l=p?1/0:c,h=Date.now()-x;if(h>l){i?.logger.warn(`Auto-reconnect window exhausted after ${O} attempts`),dt(h,l);return}p&&h>6e5&&i?.emit("error:reconnecting",{attempt:O+1,delayMs:0,elapsedMs:h,windowMs:l,longOutage:!0});let m=r.reconnectBaseDelayMs??2e3,v=r.reconnectMaxDelayMs??3e4,_=Math.min(m*Math.pow(2,O),v),Y=_>=v,Z=p&&Y?3e4:Math.round(_*(.7+Math.random()*.3));i?.logger.info(`Scheduling auto-reconnect attempt ${O+1} in ${Z}ms`),i?.emit("error:reconnecting",{attempt:O+1,delayMs:Z,elapsedMs:h,windowMs:l}),N=setTimeout(()=>{N=null,pt()},Z)},ne=p=>{r.autoReconnect===!1||!a||!((i?.getState("live")??!1)&&de)&&!L||p.type!=="network"&&p.type!=="media"||(x===0&&(x=Date.now(),R=o?.currentTime??0,F=p),ye())},pt=async()=>{if(!i||!a)return;ie=!0;let p=++w;O++;let c=a,l=i.getState("live"),h=u,g=R;i.logger.info(`Auto-reconnect attempt ${O}`,{src:j(c)});try{if(ge(new Error("HLS load cancelled: reconnecting")),k=0,C=0,re=0,f=0,a=c,i.setState("playbackState","loading"),h&&n.supportsNativeHLS()?await ee(c):await Ie(c),p!==w)return;if(!l&&o&&g>0&&(o.currentTime=g),i.setState("playbackState","ready"),i.setState("buffering",!1),i.emit("error:recovered",{attempt:O,elapsedMs:Date.now()-x}),i.logger.info("Auto-reconnect succeeded"),te(),i.getState("paused"))i.logger.info("Stream reconnected while paused; maintaining pause at live edge");else try{await o?.play()}catch{}}catch{if(p!==w)return;i?.logger.warn(`Auto-reconnect attempt ${O} failed`),ye()}finally{ie=!1}};return{id:"hls-provider",name:e.name,version:rr,type:"provider",description:e.description,canPlay(p){if(!n.isHLSSupported())return!1;let c=p.toLowerCase();return!!(c.split("?")[0].split("#")[0].endsWith(".m3u8")||c.includes("application/x-mpegurl")||c.includes("application/vnd.apple.mpegurl"))},async init(p){i=p,i.logger.info(`HLS plugin${e.logSuffix} initialized`),q=er(i);let c=i.on("playback:play",async()=>{if(o&&o.paused)try{A.corePlayRequested=!0,await o.play()}catch(M){A.corePlayRequested=!1,i?.logger.error("Play failed",M)}}),l=i.on("playback:pause",()=>{if(o){if(o.paused){A.corePlayRequested&&(A.corePlayRequested=!1,o.pause());return}A.corePauseRequested=!0,A.corePlayRequested=!1,o.pause()}}),h=i.on("playback:seeking",({time:M})=>{if(!o||!Number.isFinite(M))return;let oe=Math.max(0,Math.min(M,o.duration||0));o.currentTime=oe}),g=i.on("volume:change",({volume:M})=>{o&&(o.volume=M)}),m=i.on("volume:mute",({muted:M})=>{o&&(o.muted=M)}),v=i.on("playback:ratechange",({rate:M})=>{o&&(o.playbackRate=M)}),_=i.on("quality:select",({quality:M,auto:oe})=>{if(!s||u){i?.logger.warn("Quality selection not available");return}if(oe||M==="auto")P=!0,s.currentLevel=-1,i?.logger.debug("Quality: auto selection enabled"),i?.setState("currentQuality",{id:"auto",label:"Auto",width:0,height:0,bitrate:0,active:!0});else{P=!1;let ae=parseInt(M.replace("level-",""),10);if(!isNaN(ae)&&ae>=0&&ae<s.levels.length){s.nextLevel=ae,i?.logger.debug(`Quality: queued switch to level ${ae}`);let be=s.levels[ae];if(be){let ht=fe(be);i?.setState("currentQuality",{id:`level-${ae}`,label:`${ht}...`,width:be.width,height:be.height,bitrate:be.bitrate,active:!1})}}}}),Y=i.on("track:audio",({trackId:M})=>{if(!s||u){i?.logger.warn("Audio track selection not available");return}let oe=Jt(M),ae=s.audioTracks??[];if(oe<0||oe>=ae.length){i?.logger.warn("Ignoring unknown audio track selection",{trackId:M});return}s.audioTrack=oe,i?.logger.debug(`Audio: queued switch to track ${oe}`)});typeof window<"u"&&(S=()=>{(N!==null||x>0||ie||O>0&&!$)&&(N&&(i?.logger.info("Browser back online, reconnecting immediately"),clearTimeout(N),N=null),pt())},window.addEventListener("online",S));let Z=i.subscribeToState(M=>{M.key==="poster"&&Te()}),se=i.subscribeToState(M=>{M.key==="live"&&(de=!0)});i.onDestroy(()=>{c(),l(),h(),g(),m(),v(),_(),Y(),Z(),se()});let J=i.subscribeToState(M=>{M.key==="playing"&&(M.value?ut():ve())});i.onDestroy(J)},async destroy(){i?.logger.info(`HLS plugin${e.logSuffix} destroying`),w++,te(),S&&typeof window<"u"&&(window.removeEventListener("online",S),S=null),ke(new Error("HLS load cancelled: player destroyed")),q?.destroy(),q=null,o?.parentNode&&o.parentNode.removeChild(o),o=null,i=null},async loadSource(p){if(!i)throw new Error("Plugin not initialized");i.logger.info(`Loading HLS source${e.logSuffix}`,{src:j(p)});let c=++w;if(te(),L=!1,de=!1,i.setState("live",!1),ke(new Error("HLS load cancelled: superseded by a new load")),a=p,A.corePlayRequested=!1,A.corePauseRequested=!1,Te(),i.setState("playbackState","loading"),i.setState("buffering",!0),i.getState("airplayActive")&&n.supportsNativeHLS())i.logger.info("Using native HLS (AirPlay active)"),await ee(p);else if(n.isHlsJsSupported())i.logger.info(`Using ${e.engineLabel} for HLS playback`),await Ie(p);else if(n.supportsNativeHLS())i.logger.info("Using native HLS playback (hls.js not supported)"),await ee(p);else throw new Error("HLS playback not supported in this browser");if(c===w){if(o){let l=i.getState("muted"),h=i.getState("volume");l!==void 0&&(o.muted=l),h!==void 0&&(o.volume=h)}i.setState("playbackState","ready"),i.setState("buffering",!1)}},getCurrentLevel(){return u||!s?-1:s.currentLevel},setLevel(p){if(u||!s){i?.logger.warn("Quality selection not available in native HLS mode");return}s.currentLevel=p},getLevels(){return u||!s?[]:Yt(s.levels,s.currentLevel)},getHlsInstance(){return s},isNativeHLS(){return u},getLiveInfo(){if(!(i?.getState("live")||!1))return null;let c=Se();if(u){let h=c?.targetLatency??3,g=o?.seekable?.length?o.seekable.end(o.seekable.length-1):void 0;return{isLive:!0,latency:c?.latency??0,targetLatency:h,drift:0,liveSyncPosition:g!==void 0?Math.max(0,g-h):void 0,lowLatency:c?.lowLatency??!1}}if(!s)return null;let l=s.targetLatency||c?.targetLatency||3;return{isLive:!0,latency:s.latency||0,targetLatency:l,drift:s.drift||0,liveSyncPosition:s.liveSyncPosition??(o?.seekable?.length?Math.max(0,o.seekable.end(o.seekable.length-1)-l):void 0),lowLatency:c?.lowLatency??!1}},async switchToNative(){if(u){i?.logger.debug("Already using native HLS");return}if(!n.supportsNativeHLS()){i?.logger.warn("Native HLS not supported in this browser");return}if(!a){i?.logger.warn("No source loaded");return}i?.logger.info("Switching to native HLS for AirPlay");let p=i?.getState("playing")||!1,c=o?.currentTime||0,l=a,h=H,g=++w;if(te(),ke(new Error("HLS load cancelled: switching to native HLS")),a=l,H=h,await ee(l),g===w){if(o&&c>0&&(o.currentTime=c),p&&o)try{await o.play()}catch{i?.logger.debug("Could not auto-resume after switch")}i?.logger.info("Switched to native HLS")}},async switchToHlsJs(){if(!u){i?.logger.debug("Already using hls.js");return}if(!n.isHlsJsSupported()){i?.logger.warn("hls.js not supported in this browser");return}if(!a){i?.logger.warn("No source loaded");return}i?.logger.info("Switching back to hls.js");let p=i?.getState("playing")||!1,c=o?.currentTime||0,l=a,h=H,g=++w;if(te(),ke(new Error("HLS load cancelled: switching to hls.js")),a=l,H=h,await Ie(l),g===w){if(o&&c>0&&(o.currentTime=c),p&&o)try{await o.play()}catch{i?.logger.debug("Could not auto-resume after switch")}i?.logger.info("Switched to hls.js")}}}}function nr(n){return ir(Ct,{name:"HLS Provider",description:"HLS playback provider using hls.js",logSuffix:"",engineLabel:"hls.js"},n)}var sr=typeof __PKG_VERSION__<"u"?__PKG_VERSION__:"0.0.0-dev";var Qr=["mp4","webm","mov","mkv","ogv","m4v"],or=["mp3","wav","ogg","flac","aac","m4a","opus","weba"],Yr=[...Qr,...or],bt={ABORTED:1,NETWORK:2,DECODE:3,SRC_NOT_SUPPORTED:4},jr={mp4:"video/mp4",m4v:"video/mp4",webm:"video/webm",mov:"video/quicktime",mkv:"video/x-matroska",ogv:"video/ogg",mp3:"audio/mpeg",wav:"audio/wav",ogg:"audio/ogg",flac:"audio/flac",aac:"audio/aac",m4a:"audio/mp4",opus:"audio/opus",weba:"audio/webm"};function ar(n){let e=n?.preload??"metadata",t=n?.loadTimeoutMs??3e4,r=null,i=null,s=null,o=null,u=!1,a=!1,d=!1,y=0,P=null,A=!1,q=f=>{try{return new URL(f,window.location.href).pathname.split(".").pop()?.toLowerCase()??""}catch{return(f.split(".").pop()?.toLowerCase()??"").split("?")[0]??""}},T=f=>jr[f]||"video/mp4",I=f=>or.includes(f),H=f=>{let L=(f.startsWith("audio/")?document.createElement("audio"):document.createElement("video")).canPlayType(f);return L==="probably"||L==="maybe"},w=()=>{if(i){if(u){i.poster="";return}i.poster=r?.getState("poster")||""}},W=()=>{if(i)return i;let f=r?.container.querySelector("video");return f?(i=f,i):(i=document.createElement("video"),i.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;display:block;object-fit:contain;background:#000",i.preload=e,i.controls=!1,i.playsInline=!0,w(),r?.container.appendChild(i),i)},k=f=>{switch(f?.code){case bt.ABORTED:return{code:"PLAYBACK_FAILED",message:"Playback aborted"};case bt.NETWORK:return{code:"MEDIA_NETWORK_ERROR",message:"Network error"};case bt.DECODE:return{code:"MEDIA_DECODE_ERROR",message:"Decode error - format may not be supported"};case bt.SRC_NOT_SUPPORTED:return A?{code:"MEDIA_NETWORK_ERROR",message:"Network error"}:{code:"SOURCE_LOAD_FAILED",message:"Format not supported"};default:return{code:"PLAYBACK_FAILED",message:"Unknown video error"}}},C=f=>{let V=[],G=y,L=(x,R)=>{let S=F=>{G===y&&R(F)};f.addEventListener(x,S),V.push([x,S])},N=()=>{f.ended||!r?.getState("ended")||(r?.setState("ended",!1),r?.setState("playbackState",f.paused?"paused":"playing"))};L("play",()=>{r?.setState("paused",!1),N()}),L("playing",()=>{r?.setState("playing",!0),r?.setState("paused",!1),r?.setState("playbackState","playing"),N(),d=!1,a?a=!1:r?.emit("playback:play",void 0)}),L("pause",()=>{a=!1,r?.setState("playing",!1),r?.setState("paused",!0),r?.setState("playbackState","paused"),d?d=!1:r?.emit("playback:pause",void 0)}),L("ended",()=>{r?.setState("playing",!1),r?.setState("ended",!0),r?.setState("playbackState","ended"),r?.emit("playback:ended",void 0)}),L("timeupdate",()=>{r?.setState("currentTime",f.currentTime),r?.emit("playback:timeupdate",{currentTime:f.currentTime})}),L("durationchange",()=>{r?.setState("duration",f.duration||0)}),L("loadedmetadata",()=>{r?.setState("duration",f.duration||0),r?.emit("media:loadedmetadata",{duration:f.duration||0})}),L("canplay",()=>{r?.setState("buffering",!1),r?.emit("media:canplay",void 0)}),L("canplaythrough",()=>{r?.emit("media:canplaythrough",void 0)}),L("waiting",()=>{r?.setState("buffering",!0),r?.emit("media:waiting",void 0)}),L("progress",()=>{if(f.buffered.length>0){let x=f.buffered.end(f.buffered.length-1),R=f.duration||0,S=R>0?x/R:0;r?.setState("bufferedAmount",S),r?.emit("media:progress",{buffered:S})}}),L("seeking",()=>{r?.setState("seeking",!0),N()}),L("seeked",()=>{r?.setState("seeking",!1),r?.emit("playback:seeked",{time:f.currentTime})}),L("volumechange",()=>{r?.setState("volume",f.volume),r?.setState("muted",f.muted),r?.emit("volume:change",{volume:f.volume,muted:f.muted})}),L("ratechange",()=>{r?.setState("playbackRate",f.playbackRate),r?.emit("playback:ratechange",{rate:f.playbackRate})}),L("stalled",()=>{r?.setState("buffering",!0),r?.emit("media:stalled",void 0),r?.logger.warn("Media stalled - network may be slow")}),L("suspend",()=>{r?.emit("media:suspend",void 0)}),L("abort",()=>{r?.emit("media:abort",void 0)}),L("error",()=>{let x=f.error,{code:R,message:S}=k(x);r?.logger.error("Video error",{mediaErrorCode:x?.code,code:R,message:S}),r?.emit("error",{code:R,message:S,fatal:!0,timestamp:Date.now()})}),L("enterpictureinpicture",()=>{r?.setState("pip",!0),r?.logger.debug("PiP: entered (standard)")}),L("leavepictureinpicture",()=>{r?.setState("pip",!1),r?.logger.debug("PiP: exited (standard)"),(!f.paused||r?.getState("playing"))&&f.play().catch(()=>{})});let O=f;return"webkitPresentationMode"in f&&L("webkitpresentationmodechanged",()=>{let x=O.webkitPresentationMode;r?.setState("pip",x==="picture-in-picture"),r?.logger.debug(`PiP: mode changed to ${x} (webkit)`),x==="inline"&&f.paused&&f.play().catch(()=>{})}),()=>{V.forEach(([x,R])=>{f.removeEventListener(x,R)})}},U=()=>{s?.(),s=null,a=!1,d=!1,i&&(i.pause(),i.removeAttribute("src"),i.load())};return{id:"native-provider",name:"Native Media Provider",version:sr,type:"provider",description:"Native HTML5 playback for video (MP4, WebM, MOV) and audio (MP3, WAV, FLAC, AAC)",canPlay(f){let V=q(f);if(!Yr.includes(V))return!1;let G=T(V);return H(G)},async init(f){r=f,r.logger.info("Native video plugin initialized");let V=r.on("playback:play",async()=>{if(!r?.getState("chromecastActive")&&i&&i.paused)try{a=!0,await i.play()}catch(S){a=!1,r?.logger.error("Play failed",S)}}),G=r.on("playback:pause",()=>{if(!r?.getState("chromecastActive")&&i){if(i.paused){a&&(a=!1,i.pause());return}d=!0,a=!1,i.pause()}}),L=r.on("playback:seeking",({time:S})=>{if(r?.getState("chromecastActive")||!i||!Number.isFinite(S))return;let F=Math.max(0,Math.min(S,i.duration||0));i.currentTime=F}),N=r.on("volume:change",({volume:S,muted:F})=>{i&&(i.volume=S,i.muted=F)}),O=r.on("volume:mute",({muted:S})=>{i&&(i.muted=S)}),x=r.on("playback:ratechange",({rate:S})=>{i&&(i.playbackRate=S)}),R=r.subscribeToState(S=>{S.key==="poster"&&w()});r.onDestroy(()=>{V(),G(),L(),N(),O(),x(),R()})},async destroy(){r?.logger.info("Native video plugin destroying"),y++;let f=P;P=null,f?.(new Error("Player destroyed during load")),U(),i?.parentNode&&i.parentNode.removeChild(i),i=null,r=null,o=null,u=!1,A=!1},async loadSource(f){if(!r)throw new Error("Plugin not initialized");let V=q(f),G=T(V),L=I(V);u=L,r.logger.info("Loading native media source",{src:j(f),mimeType:G,isAudio:L});let N=++y,O=P;if(P=null,O?.(new Error("Load superseded by a newer source")),A=!1,U(),r.setState("playbackState","loading"),r.setState("buffering",!0),r.setState("mediaType",L?"audio":"video"),L){let R=r.getState("title");if(!R||R===o)try{let F=new URL(f,window.location.href).pathname.split("/").pop()||"Audio",$=decodeURIComponent(F.replace(/\.[^.]+$/,"").replace(/[-_]/g," "));o=$,r.setState("title",$)}catch{o="Audio",r.setState("title","Audio")}}r.setState("qualities",[]),r.setState("currentQuality",null);let x=W();return x.style.display=L?"none":"block",w(),s=C(x),new Promise((R,S)=>{let F=null,$=()=>{x.removeEventListener("loadedmetadata",de),x.removeEventListener("error",X),F!==null&&(clearTimeout(F),F=null),P===ie&&(P=null)},ie=Q=>{$(),S(Q)};P=ie;let de=()=>{if(N!==y)return;$(),A=!0;let Q=r?.getState("muted"),he=r?.getState("volume");Q!==void 0&&(x.muted=Q),he!==void 0&&(x.volume=he),r?.setState("source",{src:f,type:G}),r?.setState("playbackState","ready"),r?.setState("buffering",!1),r?.emit("media:loaded",{src:f,type:G}),R()},X=()=>{if(N!==y)return;$();let Q=x.error;S(new Error(Q?.message||"Failed to load video source"))};t>0&&(F=setTimeout(()=>{N===y&&($(),S(new Error("Video took too long to load (network timeout)")))},t)),x.addEventListener("loadedmetadata",de),x.addEventListener("error",X),x.src=f,x.load()})}}}var lr={"bandwidth-indicator":{rank:0,exit:"hide"},"skip-backward":{rank:1,exit:"overflow"},"skip-forward":{rank:1,exit:"overflow"},pip:{rank:2,exit:"overflow"},chromecast:{rank:4,exit:"overflow"},airplay:{rank:4,exit:"overflow"},volume:{rank:5,exit:"overflow"},captions:{rank:6,exit:"overflow"},quality:{rank:6,exit:"hide"},time:{rank:7,exit:"hide"},play:{rank:"never",exit:"overflow"},"live-indicator":{rank:"never",exit:"overflow"},settings:{rank:"never",exit:"overflow"},fullscreen:{rank:"never",exit:"overflow"},spacer:{rank:"never",exit:"overflow"}};function Et(n,e){return n.map(t=>{let r=lr[t],i=e?.[t]??r?.rank??3;return{id:t,rank:i,exit:r?.exit??"overflow"}})}function Ot(n,e){if(!n.includes("quality")||n.includes("settings"))return;let[t]=Et(["quality"],e);if(t.rank!=="never")throw new Error(`uiPlugin: a layout with "quality" needs "settings" as well. The quality control hides when the bar does not fit, and the settings menu is where its Quality row lives. Add "settings" to controls, pin quality with priority: { quality: 'never' }, or set responsive: false.`)}function Xr(n,e,t,r){let i=n.reduce((u,a)=>u+a,0),s=e*Math.max(0,n.length-1),o=r?t+e:0;return i+s+o}function Bt(n,e,t,r){let i=n.map(d=>d.id);if(e<=0)return{inBar:i,overflow:[],hidden:[]};let o=[...n.filter(d=>d.visible&&d.width>0)],u=new Set,a=new Set;for(;Xr(o.map(d=>d.width),t,r,u.size>0)>e;){let d=-1;for(let P=0;P<o.length;P++){let A=o[P];A.rank!=="never"&&(d===-1||A.rank<=o[d].rank)&&(d=P)}if(d===-1)break;let[y]=o.splice(d,1);(y.exit==="hide"?a:u).add(y.id)}return{inBar:i.filter(d=>!u.has(d)&&!a.has(d)),overflow:i.filter(d=>u.has(d)),hidden:i.filter(d=>a.has(d))}}var Vt=`
/* ============================================
   Container & Base
   ============================================ */
.sp-container {
