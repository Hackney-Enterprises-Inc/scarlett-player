/**
 * Version reporting for @scarlett-player/native.
 *
...
describe('@scarlett-player/native version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createNativePlugin().version).toBe(PKG_VERSION);
    expect(createNativePlugin().version).toMatch(SEMVER);
  });
});
