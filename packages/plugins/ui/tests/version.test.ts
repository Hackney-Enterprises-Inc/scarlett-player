/**
 * Version reporting for @scarlett-player/ui.
 *
...
describe('@scarlett-player/ui version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(uiPlugin().version).toBe(PKG_VERSION);
    expect(uiPlugin().version).toMatch(SEMVER);
  });
});
