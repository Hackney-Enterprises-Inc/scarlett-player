/**
 * Version reporting for @scarlett-player/chapters.
 *
...
describe('@scarlett-player/chapters version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createChaptersPlugin().version).toBe(PKG_VERSION);
    expect(createChaptersPlugin().version).toMatch(SEMVER);
  });
});
