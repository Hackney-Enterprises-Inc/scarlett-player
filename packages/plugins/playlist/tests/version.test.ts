/**
 * Version reporting for @scarlett-player/playlist.
 *
...
describe('@scarlett-player/playlist version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createPlaylistPlugin().version).toBe(PKG_VERSION);
    expect(createPlaylistPlugin().version).toMatch(SEMVER);
  });
});
