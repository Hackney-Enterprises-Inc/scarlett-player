/**
 * Version reporting for @scarlett-player/gestures.
 *
...
describe('@scarlett-player/gestures version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createGesturesPlugin().version).toBe(PKG_VERSION);
    expect(createGesturesPlugin().version).toMatch(SEMVER);
  });
});
