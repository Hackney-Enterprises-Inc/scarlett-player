/**
 * Version reporting for @scarlett-player/chromecast.
 *
...
describe('@scarlett-player/chromecast version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(chromecastPlugin().version).toBe(PKG_VERSION);
    expect(chromecastPlugin().version).toMatch(SEMVER);
  });
});
