/**
 * Version reporting for @scarlett-player/media-session.
 *
...
describe('@scarlett-player/media-session version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createMediaSessionPlugin().version).toBe(PKG_VERSION);
    expect(createMediaSessionPlugin().version).toMatch(SEMVER);
  });
});
