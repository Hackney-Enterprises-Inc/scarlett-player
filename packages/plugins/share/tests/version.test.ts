/**
 * Version reporting for @scarlett-player/share.
 *
...
describe('@scarlett-player/share version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createSharePlugin().version).toBe(PKG_VERSION);
    expect(createSharePlugin().version).toMatch(SEMVER);
  });
});
