/**
 * Version reporting for @scarlett-player/audio-ui.
 *
...
describe('@scarlett-player/audio-ui version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createAudioUIPlugin().version).toBe(PKG_VERSION);
    expect(createAudioUIPlugin().version).toMatch(SEMVER);
  });
});
