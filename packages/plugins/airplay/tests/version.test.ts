/**
 * Version reporting for @scarlett-player/airplay.
 *
...
describe('@scarlett-player/airplay version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(airplayPlugin().version).toBe(PKG_VERSION);
    expect(airplayPlugin().version).toMatch(SEMVER);
  });
});
