/**
 * Version reporting for @scarlett-player/captions.
 *
...
describe('@scarlett-player/captions version', () => {
  ...
  it('reports the build constant on the descriptor, not a literal', () => {
    expect(createCaptionsPlugin().version).toBe(PKG_VERSION);
    expect(createCaptionsPlugin().version).toMatch(SEMVER);
  });
});
